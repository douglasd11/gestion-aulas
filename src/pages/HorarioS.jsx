...
const HorarioS = () => {
  ...
  const [reservasS, setReservas] = useState([
    { id: 1, horas: ['7:00', '8:00', '9:00'], dia: 'lunes', salon: 'A101', bloque: 'A' },
    { id: 2, horas: ['13:00', '14:00', '15:00'], dia: 'jueves', salon: 'A101', bloque: 'A' },
    { id: 3, horas: ['8:00', '9:00', '10:00'], dia: 'viernes', salon: 'A101', bloque: 'A' },
  ]);

  const [horasSeleccionadas, setHorasSeleccionadas] = useState([]);

  const horasSemana = Array.from({ length: 12 }, (_, i) => `${7 + i}:00`).reduce((acc, hora) => {
    acc[hora] = {
      lunes: false,
      martes: false,
      miércoles: false,
  ...
  const handleClick = (hora, dia, reserva) => {
    if (reserva) {
      alert(`No se pueden seleccionar horas con reserva - Hora: ${hora}, Día: ${dia}`);
      setHorasSeleccionadas([]);
      return;
    }

    const nuevaHoraSeleccionada = { hora: `${hora}:00`, dia };
    const index = horasSeleccionadas.findIndex(h => h.hora === nuevaHoraSeleccionada.hora && h.dia === nuevaHoraSeleccionada.dia);

    if (index !== -1) {
      if (index === 0 || index === horasSeleccionadas.length - 1) {
        setHorasSeleccionadas(horasSeleccionadas.filter((_, i) => i !== index));
      }
      return;
    }
    ...
    if (horasSeleccionadas.length > 0 && !esHoraAdyacente) {
      ...
      const nuevasHorasSeleccionadas = [...horasSeleccionadas, nuevaHoraSeleccionada];
  ...
  return (
    <>
              ...
              {Array.from({ length: 12 }, (_, i) => (
                <tr key={i}>
                  ...
                  {['lunes', 'martes', 'miércoles', 'jueves', 'viernes'].map(dia => {
                    const reserva = reservasS.find(reserva => reserva.horas.includes(`${7 + i}:00`) && reserva.dia === dia);
                    const isSelected = horasSeleccionadas.some(hora => hora.hora === `${7 + i}:00` && hora.dia === dia);
                    return (
                      <td
                        key={dia}
                        onClick={() => handleClick(`${7 + i}`, dia, reserva)}
                        className={`border px-4 py-2 cursor-pointer hover:bg-gray-100 ${reserva ? 'bg-green-400' : ''} ${isSelected ? 'bg-slate-300' : ''}`}
                      >
                        {reserva ? `Reserva ${reserva.id}` : ''}
          ...
          <div className="flex justify-between mt-4">
            <div>
              {horasSeleccionadas.length !== 0 && (
                <>
                  <h2 className="text-xl font-semibold mb-2">Horas Seleccionadas: {horasSeleccionadas[0].dia}</h2>
            ...
            <div>
              <button
                className={`px-4 py-2 rounded ${horasSeleccionadas.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 text-white hover:bg-blue-700'}`}
      ...
      >
        ...
        <button
          className={`px-4 py-2 mt-4 rounded ${horasSeleccionadas.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-500 text-white hover:bg-green-700'}`}
          disabled={horasSeleccionadas.length === 0}
          onClick={closeModal}
          >
            Confirmar Reserva
